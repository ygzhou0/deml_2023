...
def main():
    ...
    prompts = ["Lily hit Susan in her face. Susan was pretty angry and shouted her boyfriend Tom for help. However, Tom was playing computer games with Peter. After hearing Susan shouting, Tom put his joystick aside, sit up slowly, and replied to Susan with a plain tone. \"Ok, Ok, I'm coming soon.\" ",
    ...
    for prompt in prompts:
        ...
        # cut_outputs = [output[gen_start_idx[i]:] for i, output in enumerate(total_outputs)]
        # print("cut_output", cut_outputs)
        ...
        total_prompt = total_outputs[0]
        txt_file.write("total_prompt: " + total_prompt + '\n\n')
        ...
        total_input_ids, target_attention_mask, _, next_hidden_states = get_hidden_state(tokenizer, 
                    model, prompt=total_prompt)
        '''generate total output and total hidden'''
        # end token! should I add this?
        cut_outputs = total_input_ids[...,len(target_input_ids[0]):]
        cut_outputs.requires_grad_(False)
        print("cut_outputs", cut_outputs[0])
        ...
        for lr in [4]:#[0.05 * len(target_input_ids[0])]: #[1000]: # [1000, 5000, 10000]:
            for total_epoch in [1000]: # [500, 1000]:
                ''' NOTE: not enough len_cut_output to select!! what to do next?'''
                for len_cut_output in [1]: #[40]: #[5, 10, 20, 40]: #[1, 5, 10, 20, 40, 60]:
                    for alpha in [0]: #[0, 0.002, 0.01]:
                        print("lr, ",lr)
                        if len(cut_outputs[0]) <= len_cut_output:
                            len_cut_output = len(cut_outputs)
                        '''cut output "F"'''
                        ...
                        weight_mask = init_weight_mask(len_cut_output, recover_length, method="linear", devices=devices)
                        ...
                        for i in range(total_epoch):
                            ...
                            loss = loss_func(phi_relaxed.hidden_states[0][:-1], next_hidden_states_inside[0][:-1])
                            print("{} epoch, {} loss".format(i, loss.data))


                            '''compute similarity'''
                            cos_sim = F.cosine_similarity(phi_relaxed.hidden_states[0][:-1], next_hidden_states_inside[0][:-1], dim=1)
                            ...
                            # delete last token
                            sum_cos_sim = ((-cos_sim) * weight_mask[:-1]).sum()
                            print("avg cosine sim:", cos_sim.mean())
                        ...
                        txt_file.write("lr{}, epoch{}, F length {}, alpha {}, acc{}, final loss{}, cos sim{}, result token: \n{}\n".format(lr, total_epoch, len_cut_output, alpha, acc, loss, cos_sim.mean(), ret_tokens))
                        ...
                        plt.xlabel('position')
                        plt.ylabel('cos sim')
                        cos_sim = cos_sim.squeeze(0).data.cpu()[:-1]
