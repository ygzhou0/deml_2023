...
def main():
    ...
    for prompt_ in prompts:
        ...
        for prompt_length in [5, 10, 20, 30, 40, len(total_input_ids[0])]:
            
            position = 0
            fixed_input_embed = None
            while position + prompt_length <= len(total_input_ids[0]):
                ...
                for lr in [4]:#[0.05 * len(target_input_ids[0])]: #[1000]: # [1000, 5000, 10000]:
                    for total_epoch in [1000]: # [500, 1000]:
                        for alpha in [0]: #[0, 0.002, 0.01]:
                            ...
                            txt_file.write("lr{}, epoch{}, alpha {}, acc{}, final loss{}, cos sim{}, result token: \n{}\n".format(lr, total_epoch, alpha, acc, loss, cos_sim.mean(), ret_tokens))
                            ...
                            txt_file.write("10 {}, 20 {}, 30 {}, 40 {}\n\n".format(
                                acc_10t_cnt / np.min((10, recover_length)),
                                acc_20t_cnt / np.min((20, recover_length)),
                                acc_30t_cnt / np.min((30, recover_length)),
                                acc_40t_cnt / np.min((40, recover_length))
                                ))
