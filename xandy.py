    ...
    for prompt_ in prompts:
        ...
        for lr in [0.1]:#[0.05 * len(target_input_ids[0])]: #[1000]: # [1000, 5000, 10000]:
            for total_epoch in [5000]: # [500, 1000]:
                ...
                new_input_embed_np_x = np.random.uniform(low=-0.1, high=0.1, size=size)  # initialize with uniform random
                new_input_embed_0_x = torch.FloatTensor(new_input_embed_np_x)
                '''miracle'''
                ...
                new_input_embed_0_x = new_input_embed_0_x.unsqueeze(dim=0).type(torch.float16).to(devices[0])
                new_input_embed_0_x.requires_grad_(True)


                '''try to init input embed y'''
                # size = (len(target_input_ids[0]), 4096)
                size = (len(target_input_ids[0]) - 1, 4096)
                # means = torch.zeros(size)
                # new_input_embed_16 = torch.normal(mean=means, std=0.2)
                ...
                # use gaussian distribution may be better
                # quantify its difference. max min mean variance
                new_input_embed_np_y = np.random.uniform(low=-0.1, high=0.1, size=size)  # initialize with uniform random
                new_input_embed_16_y = torch.FloatTensor(new_input_embed_np_y)
                '''miracle'''
                # new_input_embed_16 = all_hidden_states[16][16][1]
                # print("mean", new_input_embed_16.var())
                # o=1/0
                # new_input_embed_16 = new_input_embed_16.unsqueeze(0)
                new_input_embed_16_y = new_input_embed_16_y.unsqueeze(dim=0).type(torch.float16).to(devices[0])
                new_input_embed_16_y.requires_grad_(True)




                '''optimizer'''
                optim = torch.optim.SGD([new_input_embed_16_y, new_input_embed_0_x], lr=lr)
                ...
                for i in range(part_epoch):
                    # '''add start token'''
                    new_input_embed_y = torch.cat((START_16, new_input_embed_16_y), dim=1)
                    '''then I need ||phi(relaxed(Z, T)) - phi(x*)||**2'''
                    y_inputs = {'inputs_embeds': new_input_embed_y, 'attention_mask': target_attention_mask, "use_rms_norm": True}
                    
                    model.model.layers = total_layers[16:32]
                    phi_y = model(**y_inputs)

                    new_input_embed_x = torch.cat((START_EMBED, new_input_embed_0_x), dim=1)
                    '''then I need ||phi(relaxed(Z, T)) - phi(x*)||**2'''
                    x_inputs = {'inputs_embeds': new_input_embed_x, 'attention_mask': target_attention_mask, "use_rms_norm": True}
                    
                    model.model.layers = total_layers
                    phi_x = model(**x_inputs)
                    # print(phi_y.all_hidden_states)
                    # print(len(phi_y.all_hidden_states))
                    # print(phi_y.all_hidden_states[16].shape)
                    # o=1/0
                    ...
                    # o=1/0
                    loss_1 = loss_func(phi_x.all_hidden_states[-1].type(torch.float32), next_hidden_states_32.type(torch.float32))   #〖||32𝑔𝑡−32(𝑥)||〗_2^2
                    loss_2 = loss_func(phi_x.all_hidden_states[16].type(torch.float32), new_input_embed_y.type(torch.float32))   #〖||16(𝑥)−𝑦||〗_2^2
                    loss_3 = loss_func(phi_y.all_hidden_states[-1].type(torch.float32), next_hidden_states_32.type(torch.float32))   #〖||32(𝑦)−32𝑔𝑡||〗_2^2
                    loss = loss_1 + loss_2 + loss_3
                    print("{} epoch, {} loss".format(i, loss.data))


                    '''compute similarity'''
                    cos_sim = F.cosine_similarity(phi_x.hidden_states.type(torch.float32), next_hidden_states_32.type(torch.float32), dim=-1)
                    ...
                    # sum_cos_sim.backward(inputs=[new_input_embed_0_x, new_input_embed_16_y])    # optimize by cosine sim
                    loss.backward(inputs=[new_input_embed_0_x, new_input_embed_16_y])  # optimize by MSEloss
                    ...
                    if (i+1) % 500 == 0 or i == part_epoch - 1:
                        ...
                        acc, ret_tokens = invert_embedding(torch.cat((START_EMBED, new_input_embed_0_x), dim=1), tokenizer, embed_layer, total_input_ids, invert_method='cosine')
                ...
                pickle_piece = (prompt, torch.cat((START_EMBED, new_input_embed_0_x), dim=1))
                with open("result-x-0layer-{}-{}-{}-{}-{}-{}.pickle".format(*time.localtime()), "wb") as f:
        ...
        # model.model.layers = total_layers[:16]
        next_hidden_states_16 = torch.cat((START_16, new_input_embed_16_y), dim=1)
