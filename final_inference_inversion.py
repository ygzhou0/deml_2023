...
def main():
    '''create log file'''
    log_file_name = "inversion-{}-{}-{}-{}-{}-{}-{}.log".format(*time.localtime())
    txt_file = open(log_file_name, "w")
    '''get model'''
    ...
    for prompt_ in prompts:
        ...
        for i in range(total_epoch):
            ...
            for name, module in model.named_modules():
                if name == "model.layers.59":    # for llama 65B
                    handle = module.register_forward_hook(forward_hook)
                    hook_handles.append(handle)
            phi_relaxed = model(**new_inputs)
            for handle in hook_handles:
            ...
            if torch.any(torch.isnan(cos_sim)):
                txt_file.write("found NAN in cosine similarity\n")
                break
            optim.step()
            ...
            if (i+1) % 25 == 0 or i == total_epoch - 1:
                txt_file.write("cos sim: {}\n".format(cos_sim.mean().data))

            if (i+1) % 100 == 0 or i == total_epoch - 1:
                end = time.time()
